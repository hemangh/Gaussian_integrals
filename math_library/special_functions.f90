...
contains
    ...
    subroutine cgama ( x, y, kf, gr, gi )
      ...
      real ( kind = rk ), save, dimension ( 10 ) :: a = (/ &
    ...
    subroutine chguit ( a, b, x, hu, id )
      ...
      integer m
      real ( kind = rk ) s
      real ( kind = rk ), save, dimension ( 30 ) :: t = (/ &
      ...
      real ( kind = rk ) t3
      real ( kind = rk ) t4
      real ( kind = rk ), save, dimension ( 30 ) :: w = (/ &
    ...
    subroutine cik01 ( z, cbi0, cdi0, cbi1, cdi1, cbk0, cdk0, cbk1, cdk1 )
      ...
      real ( kind = rk ), save, dimension ( 12 ) :: a = (/ &
      ...
      real ( kind = rk ) a0
      real ( kind = rk ), save, dimension ( 10 ) :: a1 = (/ &
      ...
      real ( kind = rk ), save, dimension ( 12 ) :: b = (/ &
    ...
    subroutine cjy01 ( z, cbj0, cdj0, cbj1, cdj1, cby0, cdy0, cby1, cdy1 )
      ...
      real ( kind = rk ) a0
      real ( kind = rk ), save, dimension ( 12 ) :: a1 = (/ &
      ...
      real ( kind = rk ), save, dimension ( 12 ) :: b1 = (/ &
    ...
    subroutine cpsi ( x, y, psr, psi )
      ...
      real ( kind = rk ), save, dimension ( 8 ) :: a = (/ &
    ...
    subroutine elit3 ( phi, hk, c, el3 )
      ...
      logical lb2
      real ( kind = rk ) phi
      real ( kind = rk ), dimension ( 10 ), save :: t = (/ &
      ...
      real ( kind = rk ) t1
      real ( kind = rk ) t2
      real ( kind = rk ), dimension ( 10 ), save :: w = (/ &
    ...
    !
      ...
      real ( kind = rk ), save, dimension ( 12 ) :: a = (/ &
        ...
        2.4380529699556D+01, 1.1001714026925D+02, &
        5.5133589612202D+02, 3.0380905109224D+03 /)
      real ( kind = rk ), save, dimension ( 8 ) :: a1 = (/ &
        0.125D+00, 0.2109375D+00, &
        1.0986328125D+00, 1.1775970458984D+01, &
    ...
    !
      ...
      real ( kind = rk ), save, dimension ( 16 ) :: a = (/ &
    ...
    !
      ...
      real ( kind = rk ) b1
      real ( kind = rk ), save, dimension ( 8 ) :: c = (/ &
    ...
    !
      ...
      if ( nt < 600 ) then
        xm = -1.0D+00 + 2.248485D+00 * real ( nt, kind = rk ) ** 0.5D+00 &
          - 0.0159382D+00 * nt + 3.208775D-04 * real ( nt, kind = rk ) ** 1.5D+00
        nm = int ( 14.5D+00 + 0.05875D+00 * nt )
        mm = int ( 0.02D+00 * nt ) + 6
      else
        xm = 5.0D+00 + 1.445389D+00 * ( real ( nt, kind = rk ) ) ** 0.5D+00 &
          + 0.01889876D+00 * nt &
      ...
      do i = 1,nm
        ...
        x1 = 0.407658D+00 + 0.4795504D+00 &
          * ( real ( i - 1, kind = rk ) ) ** 0.5D+00 + 0.983618D+00 * ( i - 1 )
        x2 = 1.99535D+00 + 0.8333883 * ( real ( i - 1, kind = rk ) ) ** 0.5D+00 &
          + 0.984584D+00 * ( i - 1 )
        l1 = 0
    ...
    !
    !    Output, real ( kind = rk ) R8_GAMMA_LOG, the value of the function.
    !
      implicit none
